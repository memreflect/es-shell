...
extern Tree *
thunkify(Tree *tree) {
	if (tree != NULL && ((tree->kind == nThunk) || (tree->kind == nList && tree->CAR->kind == nThunk && tree->CDR == NULL)))
		return tree;
...
mkmatch(Tree *subj, Tree *cases) {
	 ...
	 * it's like saying {if}, which simply returns ltrue.
...
extern Tree *
firstprepend(Tree *first, Tree *args) {
	Tree  *t;
	Tree **tp;
	assert(first != NULL);
